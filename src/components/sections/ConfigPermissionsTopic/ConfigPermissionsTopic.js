...
class ConfigPermissionsTopic extends ControllerComponent {
	static propTypes = {
	...
	buildState(props) {
		...
		if(props.selectorValue) {
			...
			if (topic) {
				...
				topic.permissions.group.forEach(permission => {
				...
				topic.permissions.user.forEach(permission => {
					if(permission.permission == 'GET') {
	...
	onChangeObjectSelectUser(stateKey, objectType, operationType, value, values) {
		...
		if(current.length < newOnes.length) {
			...
			toAdd.forEach(id => {
				topic.addPermission(ObjectTypes.USER, id, permission);
	...
	onChangeObjectSelectGroup(stateKey, objectType, operationType, value, values) {
		...
		if(current.length < newOnes.length) {
			let toAdd = _.difference(newOnes, current);
			toAdd.forEach(id => {
				topic.addPermission(ObjectTypes.GROUP, id, permission);
				ActionCreator.addPermission(this.instance, id, permission);
			});
		} else if(current.length > newOnes.length) {
			let toRemove = _.difference(current, newOnes);
			toRemove.forEach(id => {
				topic.removePermission(ObjectTypes.GROUP, id, permission);
	...
	render() {
		...
		if(this.state.built) {
			ret = (
				<div>
					<div><h2>Specify the permissions towards the topic</h2></div>
					...
					<div className="frame-input-wrapper">
						<label className="container">
							Users with permission to update this topic
							<UIObjectSelect
								multi
								className="template"
					...
					<div className="frame-input-wrapper">
						<label className="container">
							Users with permission to delete this topic
							<UIObjectSelect
								multi
								className="template"
					...
					<div className="frame-input-wrapper">
						<label className="container">
							Groups with permission to see this topic
							<UIObjectSelect
								multi
								className="template"
					...
					<div className="frame-input-wrapper">
						<label className="container">
							Groups with permission to update this topic
							<UIObjectSelect
								multi
								className="template"
					...
					<div className="frame-input-wrapper">
						<label className="container">
							Groups with permission to delete this topic
							<UIObjectSelect
								multi
								className="template"
...
export default ConfigPermissionsTopic;
