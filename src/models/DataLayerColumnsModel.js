...
class DataLayerColumnsModel extends Model {
	...
	resolveForLocal(data) {
		return new Promise(function (resolve, reject) {
			...
			if(!data.hasOwnProperty("featureTypes") || !data.featureTypes.length) return resolve([]);
			if(!data.featureTypes[0].hasOwnProperty("properties")) return reject("featureTypes[0] has no properties in WFS response");
